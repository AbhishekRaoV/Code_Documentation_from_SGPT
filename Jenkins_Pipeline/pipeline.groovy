pipeline {
    ...
    stages {
        ...
        stage('Generate PS and PDF from Document.txt') {
            steps {
                script{

                        sh "a2ps -o output.ps Document.txt"
                        sh "touch output.pdf"// Convert Document.txt to PostScript (PS)
                        sh "ps2pdf output.ps output.pdf"     // Convert the PS file to PDF
                        archiveArtifacts artifacts: 'output.pdf'  // Archive the resulting PDF
